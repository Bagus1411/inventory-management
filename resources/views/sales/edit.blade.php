        ...
        <form action="{{ route('sales.update', $sales->id) }}" method="POST" id="form-create">
                                        ...
                                        <tr data-index="{{ $index }}">
                                                ...
                                                <select name="items[{{ $index }}][item_id]" class="form-select">
                                                    ...
                                                    @foreach ($items as $item)
                                                        <option value="{{ $item->id }}"
                                                            {{ $item->id == old('items.' . $index . '.item_id') ? 'selected' : '' }}>
                                            ...
                                            <td>
                                                <input type="number" name="items[{{ $index }}][price]"
                                                    value="{{ old('items.' . $index . '.price') }}" class="form-control">
                                            </td>
                                            <td>
                                                <input type="number" name="items[{{ $index }}][discount_percent]"
                                                    value="{{ old('items.' . $index . '.discount_percent') }}"
                                                    class="form-control">
                                            </td>
                                            <td>
                                                <input type="number" disabled
                                                    name="items[{{ $index }}][discount_amount]"
                                                    value="{{ old('items.' . $index . '.discount_amount') }}"
                                                    class="form-control">
                                            </td>
                                            <td>
                                                <input type="text" name="items[{{ $index }}][note]"
                                                    value="{{ old('items.' . $index . '.note') }}" class="form-control">
                                            </td>
                                            <td>
                                                <input type="number" disabled name="items[{{ $index }}][subtotal]"
                                                    value="{{ old('items.' . $index . '.subtotal') }}"
                                                    class="form-control">
                                            </td>
                                            {{-- <td>
                                        ...
                                        <tr data-index="{{ $index }}">
                                            <td>
                                                <select name="items[{{ $index }}][item_id]" class="form-select">
                                                    ...
                                                    @foreach ($items as $item)
                                                        <option value="{{ $item->id }}"
                                                            {{ $item->id == old('items.' . $index . '.item_id', $value->item_id) ? 'selected' : '' }}>
                                                            {{ $item->name }} (Stok : {{ $item->stock }})
                                                        </option>
                                                    @endforeach
                                                </select>
                                            ...
                                            <td>
                                                <input type="number" name="items[{{ $index }}][quantity]"
                                                    value="{{ old('items.' . $index . '.quantity', $value->quantity) }}"
                                                    class="form-control">
                                            </td>
                                            <td>
                                                <input type="number" name="items[{{ $index }}][price]"
                                                    value="{{ old('items.' . $index . '.price', $value->price) }}"
                                                    class="form-control">
                                            </td>
                                            <td>
                                                <input type="number" name="items[{{ $index }}][discount_percent]"
                                                    value="{{ old('items.' . $index . '.discount_percent', $value->discount_percent) }}"
                                                    class="form-control">
                                            </td>
                                            <td>
                                                <input type="number" disabled
                                                    name="items[{{ $index }}][discount_amount]"
                                                    value="{{ old('items.' . $index . '.discount_amount', $value->discount_amount) }}"
                                                    class="form-control" step="0.01">
                                            </td>
                                            <td>
                                                <input type="text" name="items[{{ $index }}][note]"
                                                    value="{{ old('items.' . $index . '.note', $value->note) }}"
                                                    class="form-control">
                                            </td>
                                            <td>
                                                <input type="number" disabled
                                                    name="items[{{ $index }}][subtotal]"
                                                    value="{{ old('items.' . $index . '.subtotal', $value->subtotal) }}"
                                                    class="form-control">
                                            </td>
                                ...
                                <tr class="total-row">
                                    ...
                                    <td>
                                        <input type="number" readonly name="total" class="form-control"
                                            value="{{ old('total', $details->sum('subtotal')) }}">
                                    </td>
                                    <td></td>
                                </tr>
                            </tbody>
                                ...
                                <div class="d-flex gap-2 align-items-center">
                                    <div class="input-group">
                                        ...
                                        <input type="number" class="form-control" name="discount_global"
                                            id="discount_global"
                                            value="{{ old('discount_global', $sales->discount_global) }}">
                                    </div>
                                    <i data-feather="minus"></i>
                                    <div class="input-group">
                                        <div class="input-group-text">Rp.</div>
                                        <input type="number" disabled class="form-control" name="discount_global2"
                                ...
                                <div class="d-flex gap-2 align-items-center">
                                    <div class="input-group">
                                        ...
                                        <input type="number" class="form-control" name="ppn" id="ppn"
                                            step="0.01" value="{{ old('ppn', $sales->ppn) }}">
                                    </div>
                                    ...
                                    <div class="input-group">
                                        <div class="input-group-text">Rp.</div>
                                        <input type="number" disabled class="form-control" name="ppn2"
                                            id="ppn2" step="0.01" value="{{ old('ppn2') }}">
                                    </div>
                    ...
                    <div class="card shadow-sm px-3">
                        <div class="card-body">
                            <div class="mb-3">
                                ...
                                <div class="input-group">
                                    <div class="input-group-text">Rp.</div>
                                    <input type="number" disabled class="form-control" name="total_item_value"
                            ...
                            <div class="mb-3">
                                ...
                                <div class="input-group">
                                    <div class="input-group-text">Rp.</div>
                                    <input type="number" disabled class="form-control" name="t_a_g_d" id="t_a_g_d"
            ...
            <div class="card shadow-sm mt-4 mb-4">
                <div class="bg-white d-flex justify-content-between align-items-center p-3">
                    {{-- Tombol kiri: Save & Back --}}
                    <div class="d-flex gap-2">
                        <button type="submit" class="btn btn-primary d-flex align-items-center gap-1">
                            <i data-feather="save"></i> Save Changes
                        </button>
                        <a href="{{ route('sales.index') }}" class="btn btn-secondary d-flex align-items-center gap-1">
                            <i data-feather="arrow-left"></i> Back
                        </a>
                    </div>

                    {{-- Tombol kanan: Delete (trigger modal) --}}
                    <button type="button" class="btn btn-danger d-flex align-items-center gap-1" data-bs-toggle="modal"
...
@section('scripts')
    <script>
        // Hitung jumlah row awal (dari DB atau old input)
        let rowIndex = {{ !empty(old('items')) ? count(old('items')) : count($details) }};

        function addRow() {
            let newRow = `
            <tr>
                <td><input type="number" name="items[${rowIndex}][quantity]" class="form-control"></td>
                <td><input type="text" name="items[${rowIndex}][price]" class="form-control"></td>
                <td><input type="text" name="items[${rowIndex}][discount_percent]" class="form-control"></td>
                <td><input type="number" disabled name="items[${rowIndex}][discount_amount]" placeholder="Rp" class="form-control" step="0.01"></td>
                <td><input type="text" name="items[${rowIndex}][note]" class="form-control"></td>
                <td><input type="number" disabled name="items[${rowIndex}][subtotal]" placeholder="Subtotal" class="form-control" step="0.01"></td>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.getElementById('add-row').addEventListener('click', function() {
                ...
                <td><input type="number" name="items[${rowIndex}][price]" placeholder="Price" class="form-control" step="1000" required></td>
